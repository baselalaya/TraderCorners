...
const DEFAULT_SYMBOLS = (process.env.QUOTES_SYMBOLS?.split(',').map(s => s.trim()).filter(Boolean) || [
  ...
  "CRUDE OIL",
  "NATURAL GAS",
  // Normalized/explicit commodity symbols to ensure delivery keys exist
  "CL",        // WTI Crude normalized key
  "XAU/USD",   // Spot gold pair
  "XAG/USD",   // Spot silver pair
  "BZ",        // Brent Crude normalized key
  "BTC/USD",
  "ETH/USD",
  // Popular US stocks (Yahoo tickers)
  "AAPL",
  "MSFT",
  "NVDA",
  "AMZN",
  "TSLA",
  // Major indices (Yahoo caret tickers)
  "^GSPC", // S&P 500
  "^NDX",  // Nasdaq 100
    ...
    if (latest.length === 0) {
      try {
        ...
        if (mayFetch) {
          ...
          // Merge Binance crypto into snapshot with 24h change, regardless of FX path success
          try {
            const syms = ["BTCUSDT","ETHUSDT","SOLUSDT"]; // extend as needed
            const now = Date.now();
            const out: any[] = [];
            // Get 24h stats in one call per symbol
            for (const s of syms) {
              const r = await fetch(`https://api.binance.com/api/v3/ticker/24hr?symbol=${s}` as any);
              if (!r.ok) continue;
              const j = await r.json();
              const last = Number(j?.lastPrice ?? j?.weightedAvgPrice ?? j?.prevClosePrice);
              const pct = Number(j?.priceChangePercent);
              const vol = Number(j?.quoteVolume ?? j?.volume);
              if (!Number.isFinite(last)) continue;
              const norm = s.replace("USDT","USD");
              out.push({ symbol: norm, bid: last, ask: last, price: last, ts: now,
                ...(Number.isFinite(pct) ? { changePercent24h: pct } : {}),
                ...(Number.isFinite(vol) ? { volume24h: vol } : {}),
              });
            }
            if (out.length) {
              // Prefer Binance for these symbols: replace if exists, else append
              const idxMap = new Map(snap.map((it, i) => [String(it.symbol).toUpperCase(), i] as const));
              for (const it of out) {
                const key = String(it.symbol).toUpperCase();
                const idx = idxMap.get(key);
                if (idx != null) {
                  snap[idx] = it; // override Yahoo with Binance data
                } else {
                  snap.push(it);
                }
              }
